...
async function actor(request: Request, identity?: IdentityRepositories): Promise<ActorContext | null> {
  ...
  if (!identity || !bearer) return null;
  const resolved = await resolveSession(identity, bearer);
  return resolved ? { userId: resolved.user.id, sessionId: resolved.session.id, roles: resolved.user.roles, transport: 'bearer' } : null;
}
...
export function mapRouter(options: MapRouterOptions = {}, identity?: IdentityRepositories): Router {
  ...
  router.get('/map/footprints', async (request, response) => {
    ...
    try { const result = await query.execute({ actor: await actor(request, identity), bounds: parsed.data, ...(parsed.data.cursor ? { cursor: parsed.data.cursor } : {}), ...(parsed.data.visibility ? { visibility: parsed.data.visibility } : {}) }); response.json(result); } catch { problem(response, request, 400, 'INVALID_REQUEST'); }
  });
