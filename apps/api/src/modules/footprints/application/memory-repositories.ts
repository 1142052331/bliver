...
export function createMemoryFootprintRepositories(): FootprintRepositories {
  ...
  return { footprints, outbox, idempotency, transactions, mediaOwnership: { async assertPublishable() { return undefined; } }, publicDetails: { async findById(id) { const record = records.get(id); return record ? { id: record.id, authorId: record.authorId, author: { name: String(record.authorId) }, displayPoint: record.displayPoint, visibility: record.visibility, locationPrecision: record.locationPrecision, publishedAt: record.publishedAt, discoveryExpiresAt: record.discoveryExpiresAt, message: record.message } : null; } } };
}
