...
export function createMemoryFootprintRepositories(): FootprintRepositories {
  ...
  const transactions: FootprintTransactionPort = {
    ...
    async delete(input) { const record = records.get(input.footprintId); if (!record || record.authorId !== input.actorId) throw new FootprintConflictError(); records.delete(input.footprintId); await outbox.append({ id: 'delete-event', type: 'FootprintDeleted', aggregateId: input.footprintId, payload: { footprintId: input.footprintId, authorId: input.actorId } }); },
  };
  return { footprints, outbox, idempotency, transactions, mediaOwnership: { async assertPublishable() { return undefined; } }, publicDetails: { async findById(id) { const record = records.get(id); return record ? { id: record.id, authorId: record.authorId, author: { name: String(record.authorId) }, displayPoint: record.displayPoint, visibility: record.visibility, locationPrecision: record.locationPrecision, publishedAt: record.publishedAt, discoveryExpiresAt: record.discoveryExpiresAt, message: record.message, ...(record.mood ? { mood: record.mood } : {}) } : null; } } };
}
