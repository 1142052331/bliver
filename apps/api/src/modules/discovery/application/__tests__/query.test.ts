...
describe('DiscoveryQueryService', () => {
  ...
  it('keeps expired public records discoverable to guests', async () => { const expired = entry({ discoveryExpiresAt: new Date('2000-01-01T00:00:00.000Z') }); await expect(service([expired]).execute(base)).resolves.toMatchObject({ items: [{ id: expired.id }] }); });
  ...
  it('keeps smart pages globally chronological across region, country, and global buckets', async () => {
    const region = entry({ publishedAt: new Date('2026-07-15T05:00:00.000Z'), regionId: 'region-a' });
    const country = entry({ publishedAt: new Date('2026-07-15T07:00:00.000Z'), regionId: 'region-b' });
    const global = entry({ publishedAt: new Date('2026-07-15T06:00:00.000Z'), regionId: 'region-c', countryCode: 'US' });
    ...
    expect(first.items.map((item) => item.id)).toEqual([country.id]);
    expect(second.items.map((item) => item.id)).toEqual([global.id]);
    expect(third.items.map((item) => item.id)).toEqual([region.id]);
  });
