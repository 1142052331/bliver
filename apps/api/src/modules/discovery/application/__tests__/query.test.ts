...
describe('DiscoveryQueryService', () => {
  ...
  it('does not expose expired public records to guests', async () => { const expired = entry({ discoveryExpiresAt: new Date('2000-01-01T00:00:00.000Z') }); await expect(service([expired]).execute(base)).resolves.toMatchObject({ items: [] }); });
