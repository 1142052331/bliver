...
export function identityRouter(repos: IdentityRepositories, config: ApiConfig): Router {
  ...
  router.post('/auth/register', rateLimit, async (request, response) => {
    ...
    try { const user = await registerUser(repos, { username: parsed.data.username, password: parsed.data.password, ...(parsed.data.email ? { email: parsed.data.email } : {}), ...(parsed.data.displayName ? { displayName: parsed.data.displayName } : {}) }); response.status(201).json({ user }); } catch (error) { const code = error instanceof IdentityError ? error.code : 'INVALID_REQUEST'; problem(response, code === 'USERNAME_TAKEN' ? 409 : 400, code, request); }
  });
  ...
  router.delete('/sessions/:sessionId', actor, async (request, response) => { const context = (request as Request & { actor: ActorContext }).actor; const sessionId = String(request.params.sessionId); const target = await repos.sessions.findById(sessionId); if (!target || target.userId !== context.userId) { problem(response, 404, 'SESSION_NOT_FOUND', request); return; } await revokeSession(repos, target.id); response.status(204).end(); });
  return router;
}
