...
async function insertEvent(client: DatabaseQueryPort, item: InteractionEvent): Promise<void> { await client.query('INSERT INTO platform.outbox_events (id,type,aggregate_id,payload) VALUES ($1,$2,$3,$4)', [item.id, item.type, item.aggregateId, JSON.stringify(item.payload)]); }
async function commitComment(client: DatabaseQueryPort, input: IdempotentCommentCommit): Promise<Comment> { const reserved = await client.query<Row>('INSERT INTO platform.idempotency_keys (actor_id,scope,key,request_hash,response) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (actor_id,scope,key) DO NOTHING RETURNING request_hash,response', [input.actorId, input.scope, input.key, input.fingerprint, JSON.stringify(input.comment)]); if (!reserved.rowCount) { const prior = await client.query<Row>('SELECT request_hash,response FROM platform.idempotency_keys WHERE actor_id=$1 AND scope=$2 AND key=$3', [input.actorId, input.scope, input.key]); const row = prior.rows[0]; if (!row || String(row.request_hash) !== input.fingerprint) throw new InteractionError('IDEMPOTENCY_CONFLICT'); return storedComment(row.response); } await insertComment(client, input.comment); await insertEvent(client, input.event); return input.comment; }

export function createPostgresInteractionRepository(db: DatabaseClient): InteractionRepository {
  const transactions = {
    async addReaction(input: { reaction: Reaction; event: InteractionEvent }) { await db.transaction(async (client) => { await client.query('INSERT INTO footprint_reactions (footprint_id, actor_id, emoji, created_at, updated_at) VALUES ($1,$2,$3,$4,$4) ON CONFLICT (footprint_id, actor_id) DO UPDATE SET emoji=EXCLUDED.emoji, updated_at=EXCLUDED.updated_at', [input.reaction.footprintId, input.reaction.actorId, input.reaction.emoji, input.reaction.createdAt]); await insertEvent(client, input.event); }); },
    async removeReaction(input: { footprintId: Reaction['footprintId']; actorId: Reaction['actorId']; event: InteractionEvent }) { await db.transaction(async (client) => { await client.query('DELETE FROM footprint_reactions WHERE footprint_id = $1 AND actor_id = $2', [input.footprintId, input.actorId]); await insertEvent(client, input.event); }); },
    async addComment(input: { comment: Comment; event: InteractionEvent }) { await db.transaction(async (client) => { await insertComment(client, input.comment); await insertEvent(client, input.event); }); },
    async deleteComment(input: { commentId: string; at: Date; event: InteractionEvent }) { await db.transaction(async (client) => { await client.query('UPDATE footprint_comments SET deleted_at=$2 WHERE id=$1 AND deleted_at IS NULL', [input.commentId, input.at]); await insertEvent(client, input.event); }); },
  };
  return {
    transactions,
    async findReaction(footprintId, actorId) { const result = await db.query<Row>('SELECT footprint_id, actor_id, emoji, created_at FROM footprint_reactions WHERE footprint_id = $1 AND actor_id = $2', [footprintId, actorId]); return result.rows[0] ? reaction(result.rows[0]) : null; },
    async saveReaction(item) { await db.query('INSERT INTO footprint_reactions (footprint_id, actor_id, emoji, created_at, updated_at) VALUES ($1,$2,$3,$4,$4) ON CONFLICT (footprint_id, actor_id) DO UPDATE SET emoji=EXCLUDED.emoji, updated_at=EXCLUDED.updated_at', [item.footprintId, item.actorId, item.emoji, item.createdAt]); },
    ...
    async appendEvent(item: InteractionEvent) { await insertEvent(db, item); },
