...
export class AuthorizedMemoryQuery implements MemoryQueryPort {
  ...
  async timeline(ownerId: UserId, viewer: ActorContext | null, cursor?: string) { const all = await this.readable(ownerId, viewer); const filtered = cursor ? all.filter((item) => item.publishedAt < cursor) : all; const items = filtered.slice(0, 50); return { items, ...(filtered.length > items.length ? { nextCursor: items.at(-1)?.publishedAt } : {}) }; }
  async photos(ownerId: UserId, viewer: ActorContext | null, cursor?: string): Promise<MediaPageDto> { const readable = await this.readable(ownerId, viewer); const ids = readable.map((item) => item.id); const assets = await this.media.listForFootprints(ids); const filtered = cursor ? assets.filter((item) => item.createdAt.toISOString() < cursor) : assets; const items = filtered.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()).slice(0, 50).map((item) => ({ assetId: item.assetId, footprintId: item.footprintId, url: item.url, createdAt: item.createdAt.toISOString() })); return { items, ...(filtered.length > items.length ? { nextCursor: items.at(-1)?.createdAt } : {}) }; }
