...
export function memoriesRouter(options: MemoryRouterOptions, identity: IdentityRepositories): Router {
  ...
  router.get('/me', actor, async (req, res) => { const current = context(req); if (!current) return; const owner = current.userId as never; res.json(await options.query.overview(owner, current)); });
  ...
  router.get('/profile/:userId/memories', async (req, res) => { const current=await optionalActor(req,identity); const owner=String(req.params.userId) as never;if(current&&current.userId!==owner)await options.query.recordVisit(owner,current.userId as never); res.json(await options.query.overview(owner, current)); });
