...
it('returns duplicate-open after a distinct-key unique race', async () => {
  ...
  await expect(repository.transactions!.createReport({ report, event, idempotency: { actorId: report.reporterId, scope: 'moderation.report', key: 'different-key', fingerprint: 'different' } })).rejects.toThrow('DUPLICATE_OPEN_REPORT');
...
describe('Postgres report transaction', () => {
  ...
  it('loads the committed winner for the same idempotency key without another mutation', async () => {
    const winner = { ...report, id: createEventId() };
    const query = vi.fn()
    ...
    const client = { query: query as unknown as DatabaseQueryPort['query'] };
    const repository = createPostgresReportRepository({ query: client.query, async transaction<T>(callback: (value: DatabaseQueryPort) => Promise<T>) { return callback(client); } } as unknown as DatabaseClient);
    await expect(repository.transactions!.createReport({ report, event, idempotency: { actorId: report.reporterId, scope: 'moderation.report', key: 'same-key', fingerprint: 'fingerprint' } })).resolves.toMatchObject({ id: winner.id });
