...
describe('Postgres outbox repository', () => {
  it('claims with row locking and marks processed or failed events', async () => {
    ...
    await repository.markFailed('event-2', 'provider down', 1_700_000_002_000, 1_700_000_003_000);
    ...
    expect(query.mock.calls[2]).toEqual([
      'UPDATE platform.outbox_events SET claimed_at = NULL, last_error = $2, available_at = $3, dead_lettered_at = $4 WHERE id = $1',
      ['event-2', 'provider down', new Date(1_700_000_002_000), new Date(1_700_000_003_000)],
    ]);
  });
});
