...
const conversationMessageEvents = new Set(['GreetingSent', 'ConversationUnlocked', 'MessageSent']);

export function createConversationOutboxConsumer(service: ConversationService, io: ConversationRealtimeEmitter) {
  return async (event: ClaimedOutboxEvent): Promise<void> => {
    if (!conversationMessageEvents.has(event.type)) return;
    const conversationId = event.payload.conversationId;
    const messageId = event.payload.messageId;
    const senderId = event.payload.senderId;
    if (typeof conversationId !== 'string' || typeof messageId !== 'string' || typeof senderId !== 'string') throw new Error('CONVERSATION_EVENT_INVALID');
    const sender = parseUserId(senderId);
    const conversation = await service.getConversation(sender, conversationId);
    let cursor: { sentAt: Date; id: string } | undefined;
    let message;
    do {
      const page = await service.history(sender, conversationId, 100, cursor);
      message = page.find((item) => item.id === messageId);
      const last = page.at(-1);
      cursor = page.length === 100 && last ? { sentAt: last.sentAt, id: last.id } : undefined;
    } while (!message && cursor);
    if (!message || message.senderId !== sender) throw new Error('CONVERSATION_MESSAGE_MISSING');
    const dto = { ...message, senderId: message.senderId, sentAt: message.sentAt.toISOString() };
    io.to(`user:${conversation.participantLowId}`).emit('conversation:message', dto);
    io.to(`user:${conversation.participantHighId}`).emit('conversation:message', dto);
  };
}

export function createConversationSocketHandlers(service: ConversationService, io: ConversationRealtimeEmitter) {
  const deliver = (conversation: { participantLowId: string; participantHighId: string }, event: string, payload: unknown): void => {
