...
const owner={id:ownerId,username:'harbor-owner',displayName:'Harbor Owner'};
...
test('owner and visitor see only authorized memories and notifications',async({page})=>{
  ...
  await page.route('**/api/v1/**',async(route)=>{const request=route.request();const path=new URL(request.url()).pathname;
    ...
    if(path==='/api/v1/users/me')return reply(route,{...owner,email:null,roles:['user']});
    if(path==='/api/v1/users')return reply(route,{items:[owner]});
  ...
  await page.goto('/me');await expect(page.getByRole('heading',{name:owner.displayName,exact:true})).toBeVisible();await expect(page.getByText('Harbor morning')).toBeVisible();await expectNoAxeViolations(page);await page.goto(`/profile/${ownerId}`);await expect(page.getByRole('heading',{name:'It is quiet here',exact:true})).toBeVisible();await page.goto('/notifications');await expect(page.getByText('1 unread')).toBeVisible();await page.getByRole('button',{name:'Mark read'}).click();await expect(page.getByText('0 unread')).toBeVisible();
});
