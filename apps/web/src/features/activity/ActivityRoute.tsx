...
import { useActivityQuery } from './api.js';
...
export function savePendingActivityAction(action: Record<string, string>): void { try { sessionStorage.setItem('bliver:pending-action', JSON.stringify({ ...action, returnTo: '/activity' })); } catch { return; } }
...
function ActivityRouteBody({ state = 'ready', items = [], page, onRetry, loadFromApi = false }: ActivityRouteProps) {
  const [query, setQuery] = useState(defaultQuery); const [scopeOpen, setScopeOpen] = useState(false); const remote = useActivityQuery(query, loadFromApi); const data = loadFromApi ? remote.data : page; const visible = loadFromApi ? (remote.data?.items ?? []) : (page?.items ?? items);
