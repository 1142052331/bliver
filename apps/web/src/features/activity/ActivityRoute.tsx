...
function ActivityRouteBody({ state = 'ready', items = [], page, onRetry, loadFromApi = false }: ActivityRouteProps) {
  ...
  const closeScope = (): void => { setScopeOpen(false); requestAnimationFrame(() => filterTrigger.current?.focus()); };
  return <section className="activity-route"><header className="activity-route__header"><div><p>Discovery</p><h1>Activity</h1><span>{labels[resolved]}</span></div><Button ref={filterTrigger} variant="secondary" aria-expanded={scopeOpen} onClick={() => setScopeOpen(true)}>Filter</Button></header><label className="activity-route__search">Search moments<input type="search" value={query.query ?? ''} onChange={(event) => { const { query: _query, cursor: _cursor, ...base } = query; void _query; void _cursor; setQuery({ ...base, ...(event.target.value ? { query: event.target.value } : {}) }); }} /></label>{scopeOpen ? <ActivityScopeSheet value={query} onChange={(value) => { const { cursor: _cursor, ...next } = value; void _cursor; setQuery(next); }} onClose={closeScope} /> : null}{state === 'empty' || (!visible.length && !remote.isLoading) ? <div className="activity-route__empty"><h2>No moments in this view</h2><p>Change the area or filters to look farther out.</p></div> : <div className="activity-route__stream">{visible.map((item) => <ActivityCard key={item.id} item={item} onAuthRequired={savePendingActivityAction} />)}</div>}{data?.nextCursor ? <div className="activity-route__more"><Button variant="secondary" disabled={remote.isFetchingNextPage} onClick={() => void remote.fetchNextPage()}>{remote.isFetchingNextPage ? 'Loading more' : 'Load more'}</Button></div> : null}</section>;
}
