...
function MapRouteBody({ state = 'ready', items = [], onRetry, loadFromApi = false }: MapRouteProps) {
  ...
  if (state === 'error' || remoteError) return <section className="map-route map-route--state"><Surface><h1>Map unavailable</h1><p>We could not load footprints.</p><Button onClick={retry}>Try again</Button></Surface></section>;
