...
export function MapViewportController({ viewport }: { readonly viewport: MapViewportBounds }): null {
  const map = useMap();
  useEffect(() => {
    map.fitBounds([[viewport.south, viewport.west], [viewport.north, viewport.east]], { animate: false });
  }, [map, viewport.east, viewport.north, viewport.south, viewport.west]);
  return null;
}
