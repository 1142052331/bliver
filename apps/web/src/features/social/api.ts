...
export async function fetchFriendRequests() { const body = await json(await fetch('/api/v1/friendships/requests', { credentials: 'include' })); const value = body as { incoming: unknown[]; outgoing: unknown[] }; return { incoming: value.incoming.map((item) => friendshipRequestDto.parse(item)), outgoing: value.outgoing.map((item) => friendshipRequestDto.parse(item)) }; }
export async function fetchBlocks() { const body = await json(await fetch('/api/v1/blocks', { credentials: 'include' })); return (body as { items: unknown[] }).items.map((item) => blockDto.parse(item)); }
export async function requestFriendship(targetUserId: string, idempotencyKey = crypto.randomUUID()) { const response = await fetch('/api/v1/friendships', { method: 'POST', credentials: 'include', headers: mutationHeaders({ 'content-type': 'application/json', 'idempotency-key': idempotencyKey }), body: JSON.stringify({ targetUserId }) }); return friendshipDto.parse(await json(response)); }
