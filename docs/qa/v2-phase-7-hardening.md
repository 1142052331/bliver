...
Implementation and hardening commits: `666d7a1`, `9732099`, `713cc6d`, `519477e`, `ee8ac80`, `5bb8602`, `c6a0d9d`, `7c5a33a`, `c5e64a4`, `25559e9`, `bdc24ab`, `8749298`, `f6fac92`, `4d326b6`, `2476331`, `f48a968`, `5a5c1cd`, `5c60568`, `63a7882`, `797d4b2`, `1b68419`, `62eadbe`, `cdb13e4`, `db10c61`, `545c11e`, `378dbd3`, `a23763c`, `b04e4bb`
...
- The canonical performance budgets are shared by bundle, API, PostGIS, Outbox, reconnect, browser Event Timing, and Lighthouse checks. Browser performance evidence runs after the complete functional Playwright gate in a dedicated one-worker, zero-retry Chromium stage. Its manifest and eight records identify one fresh isolated run; missing, stale, duplicate, mixed-run, or ordinary full-suite JSON cannot satisfy `perf:v2`. Release mode additionally requires live PostGIS and Lighthouse evidence instead of accepting static substitutes. Security gates cover request policy, dependency exceptions, secret scanning, private data redaction, and safe logs. Dependency exceptions reject invalid or expired UTC review dates while remaining valid through the stated review day.
- Request/Socket/Outbox observability, dependency counters, Sentry tags and sanitized terminal HTTP 5xx capture, health/readiness/version, and graceful shutdown are tested without logging actor IDs, message bodies, credentials, query values, or precise coordinates. HTTP 400/413 responses are not reported as server failures.
- The Outbox pump admits only one active tick, contains and observes rejected ticks, and drains the active tick before the HTTP server and database close. Worker idempotency depends on repository processed state instead of an unbounded process-local ID set. Parallel E2E retry injection is keyed and consumed by the acknowledged message ID, so one Playwright project cannot consume another project's failure.
...
| Check | Result |
| --- | --- |
| `npm.cmd run verify:v2-foundation` | PASS; architecture 749 modules / 717 dependencies; Vitest 87 files passed, 3 skipped; 344 tests passed, 7 skipped; workspace lint, typecheck, and build passed |
...
| `$env:CI='true'; npx.cmd playwright test apps/web/e2e/social-messaging.spec.ts --grep "real dual-browser Socket uses Outbox" --workers=2 --retries=0` | PASS; 4/4 viewports in one run without retries; each project injected and consumed only its acknowledged `message.id` |
| `npm.cmd run perf:v2:browser-evidence` | PASS; fresh isolated run produced reconnect 44.77/41.44/14.13/48.21 ms and INP 16/16/16/16 ms across the four viewports; schema validation accepted exactly eight records |
| `npm.cmd run lighthouse:v2` | PASS; mobile `throttlingMethod=provided`; performance score 1.0, LCP 170.336 ms, and CLS 0 |
| `$env:V2_LIGHTHOUSE_REPORT='.artifacts/lighthouse-v2.json'; npm.cmd run perf:v2` | PASS in local non-release mode; `index.js` 193,977 B gzip plus runtime 422 B; six API classes reported zero errors; Outbox lag 2,000 ms with 2 attempts; isolated reconnect maximum 48.2 ms; isolated INP maximum 16 ms; live PostGIS explicitly skipped |
...
| `$env:V2_BROWSER_EVIDENCE_DIR='test-results'; npm.cmd run perf:v2` | EXPECTED BLOCK; ordinary Playwright output has no isolated-run manifest and cannot supply reconnect or INP evidence |
...
The first unconstrained browser run exposed local contention at 16 workers. Failed tests passed unchanged at 4 workers, so the default is now deterministic at 4 local and 2 CI workers. Subsequent full runs exposed and fixed a Socket optimistic/ack assertion race, a handler-level delay that bypassed Outbox claim/processing, a real Leaflet attribution WCAG issue, missing real browser/performance gates, Windows-only Playwright commands, a Capacitor smoke output collision, missing sanitized Sentry 5xx capture, overlapping/unhandled Outbox ticks, process-local Outbox deduplication growth, cross-project retry injection, and concurrent functional-suite wall-clock contamination of reconnect evidence. The final complete run passed 120/120 without writing consumable performance records; the following isolated stage produced and validated all eight browser records, which remained present after the Capacitor browser smoke.
