...
import { authResponse, loginRequest, publicProfile, publicProfilesQuery, publicProfilesResponse, publicUser, registerRequest, refreshRequest } from './auth.js';
...
export function buildOpenApiDocument() {
  ...
  registry.register('PublicProfile', publicProfile);
  const publicProfilesSchema = registry.register('PublicProfilesResponse', publicProfilesResponse);
  ...
  registry.registerPath({ method: 'get', path: '/api/v1/users', request: { query: publicProfilesQuery }, responses: { 200: { description: 'Visible public profiles for the requested user ids', content: { 'application/json': { schema: publicProfilesSchema } } }, 400: { description: 'Invalid user ids', content: { 'application/problem+json': { schema: problemSchema } } } } });
  ...
  registry.registerPath({ method: 'post', path: '/api/v1/footprints/{footprintId}/read', request: { params: footprintParams }, responses: { 204: { description: 'Footprint marked as read for the current user' }, 401: { description: 'Authentication required', content: { 'application/problem+json': { schema: problemSchema } } }, 404: { description: 'Footprint not found or not visible', content: { 'application/problem+json': { schema: problemSchema } } } } });
